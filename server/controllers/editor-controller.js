...
exports.getBlocks = async (req, res) => {
  ...
  try {
    ...
    const result = getBlocks.map((block) => {
      const content = block.content ? JSON.parse(Buffer.from(Buffer.from(block.content).toString("utf-8"), "base64").toString("utf-8")) : null;

      const layout_design = block.layout_design ? Buffer.from(Buffer.from(block.layout_design).toString("utf-8"), "base64").toString("utf-8") : null;
...
exports.insertBlock = async (req, res) => {
  ...
  try {
    const result = await editorDB.insertBlock(
      data.page_id,
      data.block_id,
      data.block_style,
      data.design_type,
