...
exports.getBlocks = async (idx) => {
  try {
    const result = await query(
      `SELECT page_id, block_id, block_style, design_type, design_id, layout_design, content, block_order FROM blocks WHERE page_id=? ORDER BY block_order asc`,
      idx
    );
    return result;
  } catch (err) {
...
exports.getBlocksBackup = async (idx, save_time) => {
  try {
    const result = await query(
      `SELECT page_id, block_id, block_style, design_type, design_id, layout_design, content, block_order, save_time FROM block_backup WHERE page_id=? and save_time=? ORDER BY block_order asc`,
      [idx, save_time]
    );
    return result;
  } catch (err) {
...
exports.insertBlock = async (page_id, block_id, block_style, design_type, design_id, layout_design, block_order) => {
  try {
    const result = await query(
      `INSERT INTO blocks (page_id, block_id, block_style, design_type, design_id, layout_design, block_order) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [page_id, block_id, block_style, design_type, design_id, layout_design, block_order]
    );
    return result;
  } catch (err) {
    throw err;
...
exports.updateBlockDesign = async (block_id, design_type, design_id, content) => {
  try {
    const result = await query(`UPDATE blocks SET design_type=?, design_id=?, content=? WHERE block_id=?`, [
      design_type,
      design_id,
      content,
      block_id,
...
exports.updateBlockLayout = async (block_id, layout_design) => {
  try {
    ...
    const result2 = await query(`UPDATE blocks SET layout_design=? WHERE block_id=?`, [
      btoa(unescape(encodeURIComponent(JSON.stringify(getLayoutDesign)))),
      block_id,
    ]);
...
exports.saveBlock = async (page_idx, blocks, save_time) => {
  try {
    ...
    await Promise.all([
      ...
      ...blocks.map((item) =>
        query(
          `INSERT INTO block_backup (page_id, block_id, block_style, design_type, design_id, layout_design, content, block_order, save_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ...
      ...blocks.map((item) =>
        query(
          `UPDATE blocks SET block_style=?, design_type=?, design_id=?, layout_design=?, content=?, block_order=? WHERE page_id=? and block_id=?`,
...
exports.copyDesign = async (page_idx, blocks) => {
  try {
    ...
    blocks.forEach(async (item) => {
      await query(
        `INSERT INTO blocks (page_id, block_id, block_style, design_type, design_id, layout_design, content, block_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [page_idx, item.block_id, item.block_style, item.design_type, item.design_id, item.layout_design, item.content, item.block_order]
      );
    });
