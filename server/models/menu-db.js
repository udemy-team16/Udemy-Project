...
exports.deleteMenu = async (idx, order_num, parent_id, userID) => {
  try {
    ...
    if (parent_id === "undefined") {
      result2 = await query(`UPDATE menus SET order_num = order_num - 1 WHERE userID = ? and parent_id is null and order_num > ?`, [
        userID,
        order_num,
      ]);
    } else {
      result2 = await query(`UPDATE menus SET order_num = order_num - 1 WHERE userID = ? and parent_id = ? and order_num > ?`, [
        userID,
        parent_id,
        order_num,
      ]);
...
exports.insertMenu = async (data) => {
  try {
    ...
    let result;
    if (!data[0]) {
      result = await query("INSERT INTO menus (parent_id, title, link, origin_link, new_window, order_num, userID) VALUES (?, ?, ?, ?, ?, ?, ?)", [
        data[1],
        data[2],
        data[3],
        data[3],
