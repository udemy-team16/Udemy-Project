...
const Modal = ({ isOpen, setIsOpen }) => {
  ...
  const {title,link,newWindow} = inputValues
    ...
    const saveMenu = async () => {
      ...
      try {
        const res = await fetch('/api/insertMenu', {
          ...
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          body: JSON.stringify({ title, link, parent_id, new_window: newWindow }),
        });
            ...
            <div className='modal_infor_box'>
              ...
              {btn !== '메뉴' && (
                <div className='modal_page_infor'>
                  ...
                  <div>
                    <input type='checkbox' name='newWindow' checked={newWindow}  onChange={handleChange} />
                    새창 열기
                  </div>
                </div>
              )}
